...
export class Block<P extends Record<string, unknown> = Record<string, unknown>> {
  ...
  private _getChildrenAndProps(childrenAndProps: P): { props: P, children: Record<string, Block | Block[]> } {
    ...
    Object.entries(childrenAndProps).forEach(([key, value]) => {
      if (value instanceof Block || (Array.isArray(value) && value.every(item => item instanceof Block))) {
        children[key] = value as Block | Block[];
      } else {
        props[key] = value;
      }
    });
  ...
  private _registerEvents(eventBus: EventBus) {
    ...
    eventBus.on(Block.EVENTS.FLOW_CDU, (...args: unknown[]) => this._componentDidUpdate(args[0] as P, args[1] as P));
  ...
  private _makePropsProxy(props: P) {
    return new Proxy(props, {
      ...
      deleteProperty: () => {
        throw new Error('No se puede eliminar la propiedad');
      }
    });
  }
